...
struct process_metrics {
    ...
    int total_runtime;
    int facts[MAX_FACTS];
    int fact_found_ticks[MAX_FACTS];
    int intervals[MAX_FACTS];
    int min_interval;
    int max_interval;
};
...
void print_process_results(struct process_metrics *m) {
    ...
    for(int i = 0; i < WORK_ITERATIONS; i++) {
        printf(1, "Factorial %d: %d (Found at T+%d, Interval: %d ticks)\n", 
               i+1,
               m->facts[i], 
               m->fact_found_ticks[i] - m->start_tick,
               m->intervals[i]);
...
int main() {
    ...
    printf(1, "\nPriority Scheduling Test (Factorial 1-10)\n");
    printf(1, "============================================\n\n");
    ...
    for(int i = 0; i < NUM_PROCESSES; i++) {
        ...
        if(pid == 0) {  // Child process
            ...
            for(int task = 0; task < WORK_ITERATIONS; task++) {
                m.facts[task] = calculate_factorial(task + 1);
                
                heavy_compute();
                
                int current_tick = uptime();
                m.fact_found_ticks[task] = current_tick;
